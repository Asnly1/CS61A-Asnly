...
def total_weight(m):
    ...
    >>> check(HW_SOURCEFILE, 'total_weight', ['Index'])
    True
    """
    if is_planet(m):
        return size(m)
...
def quadratic(x, a, b, c):
    ...
    l, m, e = map(f, (lower_bound(x), upper_bound(x), extreme))
    if lower_bound(x) <= extreme <= upper_bound(x):
        return interval(min(l, m, e), max(l, m, e))
    else:
        return interval(min(l, m), max(l, m))
